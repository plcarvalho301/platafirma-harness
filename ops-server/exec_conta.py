...
def argv_escrita(destino: str) -> list[str]:
    """argv que grava o stdin em `destino`, criando o diretorio intermediario.

    Mora aqui, e nao inline no server, porque a medicao de aceite no host
    (`test_exec_conta_host.py`) tem de exercitar o MESMO argv que a producao usa: duas
    copias da mesma linha divergem em silencio, e a que o teste aprova deixa de ser a
    que roda.

    O caminho vai por ARGV (`$0`), nunca interpolado no texto do script: `bash -c` com
    o nome do arquivo dentro da string aceitaria `$(...)` vindo do parametro da tool.
    `set -e` porque `mkdir` que falha e `cat` que segue produz o pior resultado
    possivel - sucesso reportado, arquivo em lugar nenhum.
    """
    return ["bash", "-c", 'set -e; mkdir -p "$(dirname "$0")"; cat > "$0"', destino]
