...
- **Chapéu da linha** — `run_command("monta-sessao fabrica --chapeus")` lista o que
  existe; `monta-sessao fabrica --chapeu <slug>` serve o texto. Não vem no
  `monta_sessao` de abertura por desenho: chapéu carrega sob demanda, no giro em que a
  linha ativa. Slug igual ao que a base manda declarar na abertura — a lista sai do
  disco, então chapéu novo aparece sem ninguém avisar. Chapéu ausente não é permissão
  para inferir a régua: é pergunta fechada ao cliente daquela linha.
