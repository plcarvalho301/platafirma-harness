...
> para prod, não vira página de wiki, não vira entrada canônica. Fica no mesmo repo
> que `tokens.css`, `componentes.md` e as referências de arquitetura (ADR, macro-global,
> modelagem de org), porque é de lá que a página puxa o elemento do design system sem
> atravessar fronteira de repositório. Entrega de wireframe termina no commit.
> Pedido do dono para VER não é pedido para publicar: perguntar por qual canal.
...
- **Imagem acima de 12,5 MP sobe na wiki e não gera miniatura.** Teto do MediaWiki
  (`$wgMaxImageArea`). Tela longa estoura fácil: 1200 CSS px a `deviceScaleFactor: 2`
  com 2739 px de altura dá 2400x5478 = 13,1 MP. O `action=upload` responde `Success`
  e a página fica com `mw-broken-media`; o erro só aparece em
  `prop=imageinfo&iiurlwidth=N`, no campo `thumberror`. Conserto: screenshot de
  ELEMENTO (`elementHandle.screenshot`), uma peça por arquivo. Medido em 15/08/2026.
