...
natureza: modelo
estatuto: doutrinario
definicao: Documento curto e imutável que fixa uma decisão de construção com contexto, decisão em voz afirmativa e consequências, portando estado próprio (proposta, aceita, substituída). Um documento é registro de decisão se preserva o porquê contra mudança de contexto e se muda por substituição encadeada, nunca por edição do registro original.
...
natureza: modelo
estatuto: doutrinario
definicao: Meio de entregar valor a um cliente sem que ele carregue os custos e riscos específicos da entrega. Algo é serviço se cria valor para o cliente por si; o que habilita ou compõe a entrega sem valer sozinho para o cliente é componente de serviço, construído sobre itens de configuração.
